...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- 
    ...
    Same configuration as ISO19139 here.
      -->
  <xsl:template name="get-iso19115-1-2013-configuration">
    <xsl:copy-of select="document('config-editor.xml')"/>
  </xsl:template>


  <!-- Dispatch to the current profile mode -->
  <xsl:template name="dispatch-iso19115-1-2013">
    <xsl:param name="base" as="node()"/>
    <xsl:apply-templates mode="mode-iso19115-1-2013" select="$base"/>
  </xsl:template>


  <!-- The following templates usually delegates all to iso19139. -->
  <xsl:template name="evaluate-iso19115-1-2013">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
  ...
  <xsl:template name="evaluate-iso19115-1-2013-boolean">
    <xsl:param name="base" as="node()"/>
    <xsl:param name="in"/>
