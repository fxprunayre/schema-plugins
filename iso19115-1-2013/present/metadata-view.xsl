...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="view-with-header-iso19115-1-2013">
    <xsl:param name="tabs"/>
  ...
  <!-- ===================================================================== -->
  <!-- iso19115-1-2013-simple -->
  <xsl:template name="metadata-iso19115-1-2013view-simple" match="metadata-iso19115-1-2013view-simple">
    <!--<xsl:apply-templates mode="iso19115-1-2013-simple" select="*"/>-->

    <xsl:call-template name="view-with-header-iso19115-1-2013">
      <xsl:with-param name="tabs">
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title"
            select="/root/gui/schemas/iso19115-1-2013/strings/understandResource"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="iso19115-1-2013-simpleviewmode"
              select="
        ...
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19115-1-2013/strings/contactInfo"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="iso19115-1-2013-simpleviewmode"
        ...
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19115-1-2013/strings/mdContactInfo"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="iso19115-1-2013-simpleviewmode"
              select="mds:contact"/>
          </xsl:with-param>
        ...
        <xsl:call-template name="complexElementSimpleGui">
          <xsl:with-param name="title" select="/root/gui/schemas/iso19115-1-2013/strings/techInfo"/>
          <xsl:with-param name="content">
            <xsl:apply-templates mode="iso19115-1-2013-simpleviewmode"
              select="
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:identificationInfo/*/mri:citation/cit:CI_Citation/cit:date[1]"
    priority="10000">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19115-1-2013/strings/refDate"/>
      ...
      <xsl:with-param name="content">
        <xsl:apply-templates mode="iso19115-1-2013-simple" select=".|following-sibling::node()[name(.)='cit:date']"/>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>
  
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:identificationInfo/*/mri:extent/*/gex:temporalElement"
    priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19115-1-2013/strings/temporalRef"/>
      <xsl:with-param name="helpLink">
        <xsl:call-template name="getHelpLink">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="name" select="name(.)"/>
      ...
      <xsl:with-param name="content">
        <xsl:apply-templates mode="iso19115-1-2013-simple" select="*/mri:extent/*/gml:beginPosition
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:identificationInfo/*/gmd:resourceConstraints[1]"
    priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title" select="/root/gui/schemas/iso19115-1-2013/strings/constraintInfo"/>
      <xsl:with-param name="helpLink">
        <xsl:call-template name="getHelpLink">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="name" select="name(.)"/>
      ...
      <xsl:with-param name="content">
        <xsl:apply-templates mode="iso19115-1-2013-simple"
          select="*|following-sibling::node()[name(.)='gmd:resourceConstraints']/*"/>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>

  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:contact|mds:identificationInfo/*/mri:pointOfContact" priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:for-each select="*/cit:role/cit:CI_RoleCode/@codeListValue">
          <xsl:value-of
            select="geonet:getCodeListValue(/root/gui/schemas, 'iso19115-1-2013', 'cit:CI_RoleCode', normalize-space(.))"
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:identificationInfo/*/mri:descriptiveKeywords/mri:MD_Keywords"
    priority="90">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
      ...
      <xsl:with-param name="content">
        ...
        <xsl:if test="$type != ''">
          (<xsl:value-of
            select="/root/gui/schemas/*[name(.)='iso19115-1-2013']/codelists/codelist[@name = 'mri:MD_KeywordTypeCode']/
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:referenceSystemInfo/*/mrs:referenceSystemIdentifier">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
          <xsl:with-param name="name" select="name(.)"/>
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:resourceLineage/mrl:LI_Lineage/mrl:statement"
    priority="90">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mds:identificationInfo/*/mri:defaultLocale/lan:PT_Locale/lan:language" priority="99">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
          <xsl:with-param name="name" select="name(.)"/>
      ...
      <xsl:with-param name="content">
        <xsl:call-template name="iso19115-1-2013GetIsoLanguage">
          <xsl:with-param name="schema" select="$schema"/>
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mri:topicCategory
    " priority="98">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode"
    match="mds:identificationInfo/*/mri:extent/gex:EX_Extent/gex:geographicElement" priority="99">
    <xsl:apply-templates mode="iso19115-1-2013" select="gex:EX_GeographicBoundingBox">
      <xsl:with-param name="schema" select="$schema"/>
      <xsl:with-param name="edit" select="false()"/>
    </xsl:apply-templates>
  </xsl:template>
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="cit:graphic" priority="98">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
          <xsl:with-param name="name" select="name(.)"/>
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="*[*/@codeList]" priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
          <xsl:with-param name="name" select="name(.)"/>
      ...
      <xsl:with-param name="content">
        <xsl:apply-templates mode="iso19115-1-2013GetAttributeText" select="*/@codeListValue">
          <xsl:with-param name="schema" select="$schema"/>
          <xsl:with-param name="edit"   select="false()"/>
        </xsl:apply-templates>
      </xsl:with-param>
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="*[gco:Integer]
    " priority="99">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="*[gco:CharacterString]
    " priority="98">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="mri:spatialResolution" priority="100">
    <xsl:call-template name="simpleElementSimpleGUI">
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19115-1-2013-simpleviewmode" match="*|@*">
    <xsl:apply-templates mode="iso19115-1-2013-simpleviewmode" select="*"/>
  </xsl:template>
...
-->
        ...
        <tr>
          <td class="main">
            ...
            <span class="{translate(substring-before(current-grouping-key(), '-'), ':', '')} icon">
                <xsl:value-of select="/root/gui/schemas/iso19115-1-2013/labels/element[@name = 'gmd:protocol']/helper/option[@value=normalize-space(current-grouping-key())]"/>
            </span>
          </td>
          <td>
            <ul>
  ...
  <!-- Hide them -->
  <xsl:template mode="iso19115-1-2013-simple" match="
    geonet:*|*[@gco:nilReason='missing']|@gco:isoType" priority="99"/>
  <!-- Don't display -->
  
  <!-- these elements should be boxed -->
  <xsl:template mode="iso19115-1-2013-simple"
    ...
    <xsl:call-template name="simpleElement">
      ...
      <xsl:with-param name="content">
        ...
        <xsl:if test="normalize-space(cit:CI_Date/cit:dateType/cit:CI_DateTypeCode/@codeListValue)!=''">
          (<xsl:apply-templates mode="iso19115-1-2013GetAttributeText" select="cit:CI_Date/cit:dateType/cit:CI_DateTypeCode/@codeListValue">
            <xsl:with-param name="schema" select="$schema"/>
            <xsl:with-param name="edit"   select="false()"/>
          </xsl:apply-templates>)
        </xsl:if>
  ...
  <!-- TODO : PT_FreeText -->
  <xsl:template mode="iso19115-1-2013-simple" match="*[gco:CharacterString]" priority="2">
  ...
  <xsl:template mode="iso19115-1-2013-simple" match="gml:endPosition|gml:beginPosition|gml:timePosition" priority="2">
    <xsl:call-template name="simpleElement">
      <xsl:with-param name="id" select="generate-id(.)"/>
      <xsl:with-param name="title">
        <xsl:call-template name="getTitle">
  ...
  <xsl:template mode="iso19115-1-2013-simple" match="*[*/@codeList]">
    ...
    <xsl:call-template name="simpleElement">
      ...
      <xsl:with-param name="content">
        <xsl:variable name="empty">
          <xsl:apply-templates mode="iso19115-1-2013IsEmpty" select="."/>
        </xsl:variable>
        <xsl:if test="$empty!=''">
          <xsl:apply-templates mode="iso19115-1-2013-simple" select="*|@*"/>
        </xsl:if>
